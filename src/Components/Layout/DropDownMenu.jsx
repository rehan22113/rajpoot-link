          ...
          <Menu.Items className="absolute right-0 mt-2 w-36 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black/5 focus:outline-none">
            ...
            {data?.data.map((item)=>(
              ...
              <Menu.Item key={item._id}>
                {({ active }) => (
               <Link onClick={()=>{HandleCatFilter(item.name)}} to={`/${name.toLowerCase()}?id=${item._id}`} className={`${active ? 'bg-[#155984] text-white' : 'text-gray-900'} group flex w-full items-center rounded-md px-2 py-2 text-sm`}
                  >
                   {item.name}
                  </Link>
                )}
