    ...
    <h1 className="text-4xl font-extrabold tracking-tight text-skin-primary"> { category?.msg=="post" && posts?.length>0?`Products By ${posts && posts[0]?.category[0]?.name}`:category?.msg=="category"?`Sub-Category of ${cName} `:"No Post has Found with this Category"} </h1>
          ...
          <div className="bg-gray-900 bg-opacity-70 flex items-end pb-10 h-full">
            <div className="px-6 max-w-xl">
              ...
              <Link to={`/industry/category?iId=${iId}&cId=${item._id}/?iName=${iName}&cName=${item.name}`} className="flex items-center mt-4 text-white text-sm uppercase font-medium rounded hover:underline focus:outline-none">
