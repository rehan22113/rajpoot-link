...
const AllPost = () => {
  ...
  <div className="sm:flex sm:items-center sm:justify-between">
    ...
    <div className="flex items-center mt-4 gap-x-3">
      ...
      <button onClick={()=>Navigate("/myadmin-panel/newpost")} className="flex items-center justify-center w-1/2 px-5 py-2 text-sm tracking-wide text-white transition-colors duration-200 rounded-lg shrink-0 sm:w-auto gap-x-2  hover:bg-blue-500 bg-blue-600">
          ...
          <table className="min-w-full h-auto divide-y divide-gray-700">
            ...
            <tr key={item?._id}>
                ...
                <td className="px-12 py-4 text-sm font-medium whitespace-nowrap">
                  <div className={`inline px-3 py-1 text-sm font-normal rounded-full ${item.status?"text-emerald-500":"text-[#a3a3a3]"}  gap-x-2 bg-emerald-100/60 bg-gray-800`}>
                ...
                <td className="px-4 py-4 text-sm whitespace-nowrap">
                  ...
                  {item.category?item.category.map((cc,cIndex)=>(
                    <h4 className="text-gray-200">{cIndex} {">"} {cc.name}</h4>
                     )):"unCategorized"}
                    {/* <p className="text-gray-400">Brings all your news into one place</p> */}
                  </div>
                </td>
                <td className="px-4 py-4 text-sm whitespace-nowrap">
                <h4 className="text-gray-200">{item.principal?(item.principal.name):"No principal"}</h4>
