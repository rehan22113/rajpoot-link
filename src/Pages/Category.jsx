...
if(category){
  ...
  return (
    ...
    <div className="z-10 flex items-baseline justify-between pt-10 pb-6 border-b border-gray-200">
            <h1 className="text-4xl font-extrabold tracking-tight text-skin-primary"> { category?.msg=="post" && posts?.length>0?`Explore ${posts && posts[0]?.category[0]?.name} Products`:category?.msg=="category"?`Sub-Category of ${catName}`:"Loading Please Wait..."} </h1>
            </div>
